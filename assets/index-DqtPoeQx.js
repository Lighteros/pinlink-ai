import { d as M, g as nn } from "./index-BV35e4bv.js";
import {
  j as rn,
  s as sn,
  r as vt,
...
(function (n) {
  ...
  function i(d) {
    ...
    if (a(d)) return (p = d.code) !== null && p !== void 0 ? p : d.errorCode;
  }
  n.getErrorCode = i;
  function a(d) {
    return (
    ...
    if (
      ...
      (m.code = B.code),
        B.message && typeof B.message == "string"
          ? ((m.message = B.message), h(B, "data") && (m.data = B.data))
          : ((m.message = r(m.code)), (m.data = { originalError: l(d) }));
    } else
      (m.code = e.standardErrorCodes.rpc.internal),
        (m.message = f(d, "message") ? d.message : t),
        (m.data = { originalError: l(d) });
  ...
  function f(d, p) {
    return (
      typeof d == "object" && d !== null && p in d && typeof d[p] == "string"
    );
  }
})($e);
...
ye.standardErrors = {
  rpc: {
    ...
    resourceUnavailable: (n) =>
      C(v.standardErrorCodes.rpc.resourceUnavailable, n),
    transactionRejected: (n) =>
      C(v.standardErrorCodes.rpc.transactionRejected, n),
    methodNotSupported: (n) =>
      C(v.standardErrorCodes.rpc.methodNotSupported, n),
    limitExceeded: (n) => C(v.standardErrorCodes.rpc.limitExceeded, n),
  },
  provider: {
    userRejectedRequest: (n) =>
      $(v.standardErrorCodes.provider.userRejectedRequest, n),
    unauthorized: (n) => $(v.standardErrorCodes.provider.unauthorized, n),
    unsupportedMethod: (n) =>
      $(v.standardErrorCodes.provider.unsupportedMethod, n),
    disconnected: (n) => $(v.standardErrorCodes.provider.disconnected, n),
    chainDisconnected: (n) =>
      $(v.standardErrorCodes.provider.chainDisconnected, n),
    unsupportedChain: (n) =>
      $(v.standardErrorCodes.provider.unsupportedChain, n),
    custom: (n) => {
      if (!n || typeof n != "object" || Array.isArray(n))
        throw new Error(
...
const un = Q,
  ...
  pn = $e;
function fn(n, e) {
  const t = (0, pn.serialize)(mn(n), { shouldIncludeStack: !0 }),
    r = new URL("https://docs.cloud.coinbase.com/wallet-sdk/docs/errors");
  r.searchParams.set("version", hn.LIB_VERSION),
    r.searchParams.set("code", t.code.toString());
  const s = bn(t.data, e);
  return (
    s && r.searchParams.set("method", s),
...
function mn(n) {
  return typeof n == "string"
    ? { message: n, code: gn.standardErrorCodes.rpc.internal }
    : (0, un.isErrorResponse)(n)
...
function xt(n, e = !1) {
  ...
  throw q.standardErrors.rpc.invalidParams(
    `"${String(n)}" is not a hexadecimal string`
  );
}
...
function se(n, e = !1) {
  let t = xt(n, !1);
  return (
    t.length % 2 === 1 && (t = (0, I.HexString)(`0${t}`)),
    e ? (0, I.HexString)(`0x${t}`) : t
  );
}
...
function kn(n) {
  ...
  throw q.standardErrors.rpc.invalidParams(
    `Invalid Ethereum address: ${String(n)}`
  );
}
...
function Ln(n) {
  if (n !== null && (typeof n == "bigint" || Nt(n)))
...
function An(n) {
  ...
  throw q.standardErrors.rpc.invalidParams(
    `Not a JSON string or an object: ${String(n)}`
  );
}
...
function xn() {
  ...
  return !r || r.startsWith("javascript:") || r.startsWith("vbscript:")
    ? null
    : r.startsWith("http://") ||
      r.startsWith("https://") ||
      r.startsWith("data:")
...
y.decryptContent =
  y.encryptContent =
  y.importKeyFromHexString =
  y.exportKeyToHexString =
  y.decrypt =
  y.encrypt =
  y.deriveSharedSecret =
  y.generateKeyPair =
    void 0;
const Tt = u;
async function Nn() {
...
y.encrypt = jt;
async function Ot(n, { iv: e, cipherText: t }) {
...
const oe = y,
  Un = O,
  De = { storageKey: "ownPrivateKey", keyType: "private" },
  We = { storageKey: "ownPublicKey", keyType: "public" },
  Ue = { storageKey: "peerPublicKey", keyType: "public" };
class Bn {
  constructor() {
    (this.storage = new Un.ScopedLocalStorage("CBWSDK", "SCWKeyManager")),
  ...
  async generateKeyPair() {
    ...
    (this.ownPrivateKey = e.privateKey),
      ...
      await this.storeKey(We, e.publicKey);
  }
  async loadKeysIfNeeded() {
    if (
      (this.ownPrivateKey === null &&
        (this.ownPrivateKey = await this.loadKey(De)),
      this.ownPublicKey === null &&
        (this.ownPublicKey = await this.loadKey(We)),
      (this.ownPrivateKey === null || this.ownPublicKey === null) &&
        (await this.generateKeyPair()),
      this.peerPublicKey === null &&
        (this.peerPublicKey = await this.loadKey(Ue)),
      this.sharedSecret === null)
    ) {
      if (this.ownPrivateKey === null || this.peerPublicKey === null) return;
      this.sharedSecret = await (0, oe.deriveSharedSecret)(
        this.ownPrivateKey,
        this.peerPublicKey
      );
    }
...
class Kn {
  ...
  constructor(e) {
    var t, r;
    (this.storage = new Hn.ScopedLocalStorage("CBWSDK", "SCWStateManager")),
      (this.updateListener = e.updateListener),
      (this.availableChains = this.loadItemFromStorage(et)),
      (this._walletCapabilities = this.loadItemFromStorage(tt));
    const s = this.loadItemFromStorage(Qe),
      i = this.loadItemFromStorage(Xe);
    s &&
      this.updateListener.onAccountsUpdate({ accounts: s, source: "storage" }),
      i && this.updateListener.onChainUpdate({ chain: i, source: "storage" }),
      (this._accounts = s || []),
      (this._activeChain = i || {
        id:
          (r = (t = e.appChainIds) === null || t === void 0 ? void 0 : t[0]) !==
            null && r !== void 0
            ? r
            : 1,
      });
  ...
  updateAccounts(e) {
    (this._accounts = e),
      this.storeItemToStorage(Qe, e),
      this.updateListener.onAccountsUpdate({ accounts: e, source: "wallet" });
  }
  switchChain(e) {
    var t;
    const r =
      (t = this.availableChains) === null || t === void 0
        ? void 0
        : t.find((s) => s.id === e);
    return r
      ? (r === this._activeChain ||
          ((this._activeChain = r),
          this.storeItemToStorage(Xe, r),
...
class Fn {
  constructor(e) {
    (this.metadata = e.metadata),
      ...
      (this.keyManager = new qn.SCWKeyManager()),
      (this.stateManager = new $n.SCWStateManager({
  ...
  async handshake() {
    const e = await this.createRequestMessage({
        handshake: { method: "eth_requestAccounts", params: this.metadata },
      }),
    ...
    switch (e.method) {
      case "wallet_switchEthereumChain": {
        ...
        if (!r || !(!((t = r[0]) === null || t === void 0) && t.chainId))
          throw ce.standardErrors.rpc.invalidParams();
      ...
      case "wallet_getCapabilities": {
        ...
        if (!r)
          throw ce.standardErrors.provider.unauthorized(
  ...
  async createRequestMessage(e) {
    const t = await (0, de.exportKeyToHexString)(
      "public",
    ...
    if (!("error" in o))
      switch (e.method) {
        ...
        case "wallet_switchEthereumChain": {
          ...
          const c = e.params,
            l = (0, nt.ensureIntNumber)(c[0].chainId);
...
function ke(n) {
  if (!Buffer.isBuffer(n))
    if (Array.isArray(n)) n = Buffer.from(n);
      ...
      Ht(n) ? (n = Buffer.from(Qn(Kt(n)), "hex")) : (n = Buffer.from(n));
...
function $t(n) {
  return n.startsWith("int[")
    ...
    ? "uint256"
    : n.startsWith("fixed[")
    ? "fixed128x128" + n.slice(5)
    : n === "fixed"
    ? "fixed128x128"
    : n.startsWith("ufixed[")
    ? "ufixed128x128" + n.slice(6)
    : n === "ufixed"
    ? "ufixed128x128"
    : n;
}
...
function R(n, e) {
  ...
  if (er(n)) {
    ...
    if (((t = Ft(n)), t !== "dynamic" && t !== 0 && e.length > t))
      throw new Error("Elements exceed array size: " + t);
    (s = []),
      (n = n.slice(0, n.lastIndexOf("["))),
      typeof e == "string" && (e = JSON.parse(e));
    for (i in e) s.push(R(n, e[i]));
    if (t === "dynamic") {
    ...
    if (n === "bytes")
      return (
        ...
        e.length % 32 !== 0 &&
          (s = Buffer.concat([s, E.zeros(32 - (e.length % 32))])),
        s
      );
    if (n.startsWith("bytes")) {
      ...
      return (t = rt(n)), R("int256", H(e) * BigInt(2) ** BigInt(t[1]));
  }
...
function tr(n, e) {
  ...
  for (var i in n) {
    var a = $t(n[i]),
      o = e[i],
      c = R(a, o);
    Xn(a) ? (t.push(R("uint256", s)), r.push(c), (s += c.length)) : t.push(c);
  }
...
function Vt(n, e) {
  ...
  for (var t, r, s = [], i = 0; i < n.length; i++) {
    var a = $t(n[i]),
      o = e[i];
    if (a === "bytes") s.push(o);
    else if (a === "string") s.push(new Buffer(o, "utf8"));
    else if (a === "bool") s.push(new Buffer(o ? "01" : "00", "hex"));
    else if (a === "address") s.push(E.setLength(o, 20));
    else if (a.startsWith("bytes")) {
      if (((t = G(a)), t < 1 || t > 32))
        throw new Error("Invalid bytes<N> width: " + t);
      s.push(E.setLengthRight(o, t));
    } else if (a.startsWith("uint")) {
      if (((t = G(a)), t % 8 || t < 8 || t > 256))
        throw new Error("Invalid uint<N> width: " + t);
      r = H(o);
...
var rr = { rawEncode: tr, solidityPack: Vt, soliditySHA3: nr };
const A = qt,
  ee = rr,
  zt = {
    type: "object",
  ...
  Be = {
    encodeData(n, e, t, r = !0) {
      ...
      if (r) {
          ...
          if (t[c] !== void 0)
            return [
              ...
              l == null
                ...
                : A.keccak(this.encodeData(c, l, t, r)),
            ];
          if (l === void 0)
        ...
        for (const a of t[n]) {
          ...
          if (o !== void 0)
            if (a.type === "bytes")
              s.push("bytes32"), (o = A.keccak(o)), i.push(o);
              ...
              s.push("bytes32"),
                (o = A.keccak(this.encodeData(a.type, o, t, r))),
                i.push(o);
            else {
    ...
    encodeType(n, e) {
      ...
      for (const s of r) {
        ...
        t +=
          s +
          "(" +
          e[s].map(({ name: a, type: o }) => o + " " + a).join(",") +
          ")";
      }
      return t;
    },
    ...
    hash(n, e = !0) {
      ...
      return (
        r.push(this.hashStruct("EIP712Domain", t.domain, t.types, e)),
        t.primaryType !== "EIP712Domain" &&
          r.push(this.hashStruct(t.primaryType, t.message, t.types, e)),
        A.keccak(Buffer.concat(r))
      );
    },
  };
var sr = {
...
function ir(n) {
  ...
  return ee.soliditySHA3(
    ["bytes32", "bytes32"],
    [
      ee.soliditySHA3(new Array(n.length).fill("string"), s),
      ee.soliditySHA3(r, t),
    ]
  );
}
...
Object.defineProperty(N, "__esModule", { value: !0 });
N.APP_VERSION_KEY =
  N.LOCAL_STORAGE_ADDRESSES_KEY =
  N.WALLET_USER_NAME_KEY =
    void 0;
    ...
    return new Promise((r, s) => {
      (async function () {
        const i = await crypto.subtle.importKey(
          ...
          a = (0, le.hexStringToUint8Array)(e),
          o = a.slice(0, 12),
          c = a.slice(12, 28),
          l = a.slice(28),
          h = new Uint8Array([...l, ...c]),
          f = { name: "AES-GCM", iv: new Uint8Array(o) };
        try {
...
class pr {
  constructor({
    ...
    (this.destroyed = !1),
      ...
      (this.nextReqId = (0, F.IntNumber)(1)),
      (this._connected = !1),
      (this._linked = !1),
      ...
      (this.handleDestroyed = (a) => {
        var o;
        a === "1" &&
          ((o = this.listener) === null || o === void 0 || o.resetAndReload());
      }),
      ...
      (this.handleMetadataUpdated = async (a, o) => {
        var c;
        {
          ...
          (c = this.listener) === null ||
            c === void 0 ||
            c.metadataUpdated(a, l);
        }
      }),
      (this.handleWalletUsernameUpdated = async (a) => {
        this.handleMetadataUpdated(st.WALLET_USER_NAME_KEY, a);
      }),
      (this.handleAppVersionUpdated = async (a) => {
        this.handleMetadataUpdated(st.APP_VERSION_KEY, a);
      }),
      ...
      (this.session = e),
      (this.cipher = new ur.WalletLinkCipher(e.secret)),
      (this.listener = r);
    const i = new ue.WalletLinkWebSocket(`${t}/rpc`, s);
    i.setConnectionStateListener(async (a) => {
      ...
      i.setIncomingDataListener((a) => {
        ...
        a.id !== void 0 &&
          ((o = this.requestResolutions.get(a.id)) === null ||
            o === void 0 ||
            o(a));
      }),
      (this.ws = i),
      (this.http = new hr.WalletLinkHTTP(t, e.id, e.key));
  }
  connect() {
  ...
  set linked(e) {
    ...
    (this._linked = e),
      ...
      (r = this.listener) === null || r === void 0 || r.linkedUpdated(e);
  }
  setOnceLinked(e) {
    return new Promise((t) => {
      this.linked
  ...
  async fetchUnseenEventsAPI() {
    (this.shouldFetchUnseenEventsOnConnect = !1),
      (await this.http.fetchUnseenEvents()).forEach((t) =>
...
class Ve {
  constructor(e, t, r, s) {
    (this._storage = e),
      (this._id = t || (0, at.randomBytesHex)(16)),
      (this._secret = r || (0, at.randomBytesHex)(32)),
      (this._key = new fr.sha256()
  ...
  save() {
    return (
      this._storage.setItem(ot, this._id),
      this._storage.setItem(ct, this._secret),
      this.persistLinked(),
      this
    );
  }
...
function wr() {
  ...
  return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
    (n = window == null ? void 0 : window.navigator) === null || n === void 0
      ? void 0
      : n.userAgent
  );
}
...
function vr() {
  ...
  return (e =
    (n = window == null ? void 0 : window.matchMedia) === null || n === void 0
      ? void 0
      : n.call(window, "(prefers-color-scheme: dark)").matches) !== null &&
    e !== void 0
    ? e
    : !1;
}
...
function Sr() {
  ...
  (n.type = "text/css"),
    n.appendChild(document.createTextNode(Er.default)),
...
(function (n) {
          ...
          (0, r.h)(
            ...
            (0, r.h)(
              ...
              Array.from(this.items.entries()).map(([p, m]) =>
                (0, r.h)(n.SnackbarInstance, Object.assign({}, m, { key: p }))
              )
            )
          ),
          this.root
  ...
  const f = ({ autoExpand: d, message: p, menuItems: m }) => {
    const [B, Xt] = (0, s.useState)(!0),
      [Oe, Je] = (0, s.useState)(d ?? !1);
    (0, s.useEffect)(() => {
      const L = [
        window.setTimeout(() => {
          Xt(!1);
    ...
    return (0, r.h)(
      "div",
      {
        class: (0, t.default)(
          ...
          B && "-cbwsdk-snackbar-instance-hidden",
          Oe && "-cbwsdk-snackbar-instance-expanded"
        ),
      },
      (0, r.h)(
        "div",
        { class: "-cbwsdk-snackbar-instance-header", onClick: en },
        (0, r.h)("img", {
          src: o,
...
const Nr = ({
  ...
  return (0, x.h)(
    Ar.SnackbarContainer,
    { darkMode: t },
    (0, x.h)(
      "div",
      { class: "-cbwsdk-redirect-dialog" },
      (0, x.h)("style", null, xr.default),
      (0, x.h)("div", {
      ...
      (0, x.h)(
        "div",
        { class: (0, Mr.default)("-cbwsdk-redirect-dialog-box", i) },
        (0, x.h)("p", null, n),
        (0, x.h)("button", { onClick: r }, e)
      )
    )
  );
};
...
class Dr {
  ...
  redirectToCoinbaseWallet(e) {
    ...
    t.searchParams.append("redirect_url", (0, jr.getLocation)().href),
...
class W {
  constructor(e) {
    (this.accountsCallback = null),
      ...
      (this.isMobileWeb = (0, Hr.isMobileWeb)()),
      (this.appName = ""),
      (this.appLogoUrl = null),
      (this.linkedUpdated = (i) => {
        this.isLinked = i;
        const a = this.storage.getItem(Ur.LOCAL_STORAGE_ADDRESSES_KEY);
        if (
      ...
      (this.accountUpdated = (i) => {
        this.accountsCallback && this.accountsCallback([i]),
          W.accountRequestCallbackIds.size > 0 &&
            (Array.from(W.accountRequestCallbackIds.values()).forEach((a) => {
              const o = {
                type: "WEB3_RESPONSE",
                id: a,
                response: { method: "requestEthereumAccounts", result: [i] },
              };
              this.invokeCallback(
                Object.assign(Object.assign({}, o), { id: a })
              );
            }),
    ...
    (this._session = t),
      (this.connection = s),
      (this.relayEventManager = new Br.RelayEventManager()),
      (this.ui = r);
  }
  subscribe() {
    const e =
        He.WalletLinkSession.load(this.storage) ||
        new He.WalletLinkSession(this.storage).save(),
      { linkAPIUrl: t } = this,
      ...
      s = this.isMobileWeb
        ? new lt.WLMobileRelayUI()
        : new Kr.WalletLinkRelayUI();
    ...
    return this.sendRequest({
      ...
      params: {
        ...
        gasPriceInWei: e.gasPriceInWei
          ? (0, b.bigIntStringFromBigInt)(e.gasPriceInWei)
          : null,
    ...
    return this.sendRequest({
      ...
      params: {
        ...
        maxFeePerGas: e.maxFeePerGas
          ? (0, b.bigIntStringFromBigInt)(e.maxFeePerGas)
          : null,
        maxPriorityFeePerGas: e.maxPriorityFeePerGas
          ? (0, b.bigIntStringFromBigInt)(e.maxPriorityFeePerGas)
          : null,
        gasLimit: e.gasLimit ? (0, b.bigIntStringFromBigInt)(e.gasLimit) : null,
        chainId: e.chainId,
        shouldSubmit: !0,
  ...
  scanQRCode(e) {
    return this.sendRequest({ method: "scanQRCode", params: { regExp: e } });
  }
    ...
    const r = (0, b.randomBytesHex)(8),
      s = (i) => {
        this.publishWeb3RequestCanceledEvent(r),
          this.handleErrorResponse(r, e.method, i),
          t == null || t();
      };
    return new Promise((i, a) => {
      (t = this.ui.showConnecting({
        ...
        this.relayEventManager.callbacks.set(r, (o) => {
          if ((t == null || t(), (0, V.isErrorResponse)(o)))
    ...
    this.publishEvent("Web3Request", r, !0)
      ...
      .catch((s) => {
        this.handleWeb3ResponseMessage({
          type: "WEB3_RESPONSE",
          id: r.id,
          response: { method: t.method, errorMessage: s.message },
        });
      }),
      this.isMobileWeb && this.openCoinbaseWalletDeeplink(t.method);
  }
  openCoinbaseWalletDeeplink(e) {
    if (this.ui instanceof lt.WLMobileRelayUI)
      switch (e) {
  ...
  handleWeb3ResponseMessage(e) {
    ...
    if (t.method === "requestEthereumAccounts") {
      W.accountRequestCallbackIds.forEach((r) =>
        this.invokeCallback(Object.assign(Object.assign({}, e), { id: r }))
      ),
        W.accountRequestCallbackIds.clear();
      return;
    }
  ...
  handleErrorResponse(e, t, r) {
    var s;
    const i =
      (s = r == null ? void 0 : r.message) !== null && s !== void 0
        ? s
  ...
  requestEthereumAccounts() {
    ...
    return new Promise((r, s) => {
      this.relayEventManager.callbacks.set(t, (i) => {
        if ((0, V.isErrorResponse)(i)) return s(new Error(i.errorMessage));
        r(i);
      }),
    ...
    return (
      ...
      new Promise((f, d) => {
        this.relayEventManager.callbacks.set(l, (p) => {
          if ((c == null || c(), (0, V.isErrorResponse)(p)))
  ...
  switchEthereumChain(e, t) {
    ...
    return new Promise((i, a) => {
      this.relayEventManager.callbacks.set(s, (o) => {
        if ((0, V.isErrorResponse)(o) && o.errorCode)
          return a(
            qr.standardErrors.provider.custom({
              code: o.errorCode,
              message:
                "Unrecognized chain ID. Try adding the chain using addEthereumChain first.",
            })
          );
        if ((0, V.isErrorResponse)(o)) return a(new Error(o.errorMessage));
        i(o);
      }),
...
class Gr {
  constructor(e) {
    ...
    (this._appName = s),
      (this._appLogoUrl = i),
      (this._storage = new Zr.ScopedLocalStorage(
        "walletlink",
        ht.WALLETLINK_URL
      )),
      (this.updateListener = e.updateListener),
      (this._relayEventManager = new Vr.RelayEventManager()),
      (this._jsonRpcUrlFromOpts = "");
    const a = this._storage.getItem(ut.LOCAL_STORAGE_ADDRESSES_KEY);
    if (a) {
      const c = a.split(" ");
      c[0] !== "" &&
        ((this._addresses = c.map((l) => (0, g.ensureAddressString)(l))),
        (t = this.updateListener) === null ||
          t === void 0 ||
          t.onAccountsUpdate({ accounts: this._addresses, source: "storage" }));
    }
    this._storage.getItem(qe) &&
      ((r = this.updateListener) === null ||
        r === void 0 ||
        r.onChainUpdate({
  ...
  updateProviderInfo(e, t) {
    ...
    this._storage.setItem(qe, t.toString(10)),
      ((0, g.ensureIntNumber)(t) !== s ||
        !this.hasMadeFirstChainChangedEmission) &&
        ((r = this.updateListener) === null ||
          r === void 0 ||
          r.onChainUpdate({ chain: { id: t, rpcUrl: e }, source: "wallet" }),
        (this.hasMadeFirstChainChangedEmission = !0));
  }
  async watchAsset(e, t, r, s, i, a) {
    const c = await this.initializeRelay().watchAsset(
      e,
      t,
      r,
      s,
  ...
  async addEthereumChain(e, t, r, s, i, a) {
    ...
    return (0, D.isErrorResponse)(h)
      ? !1
      : (((o = h.result) === null || o === void 0 ? void 0 : o.isApproved) ===
          !0 && this.updateProviderInfo(t[0], e),
        ((c = h.result) === null || c === void 0 ? void 0 : c.isApproved) ===
          !0);
  }
  async switchEthereumChain(e) {
    const r = await this.initializeRelay().switchEthereumChain(
      e.toString(10),
      this.selectedAddress || void 0
    );
    if ((0, D.isErrorResponse)(r)) {
      if (!r.errorCode) return;
      throw r.errorCode === w.standardErrorCodes.provider.unsupportedChain
        ? w.standardErrors.provider.unsupportedChain()
        : w.standardErrors.provider.custom({
  ...
  _setAddresses(e, t) {
    ...
    JSON.stringify(s) !== JSON.stringify(this._addresses) &&
      ...
      (r = this.updateListener) === null ||
        r === void 0 ||
        r.onAccountsUpdate({ accounts: s, source: "wallet" }),
  ...
  _sendRequestAsync(e) {
    return new Promise((t, r) => {
      try {
        ...
        if (s !== void 0) return t({ jsonrpc: "2.0", id: e.id, result: s });
      } catch (s) {
        return r(s);
      }
      this._handleAsynchronousMethods(e)
        .then((s) => s && t(Object.assign(Object.assign({}, s), { id: e.id })))
        .catch((s) => r(s));
    });
  }
  ...
  _isKnownAddress(e) {
    try {
      ...
      return this._addresses
        .map((s) => (0, g.ensureAddressString)(s))
        .includes(t);
    } catch {}
    return !1;
  }
  ...
  _prepareTransactionParams(e) {
    const t = e.from
      ? (0, g.ensureAddressString)(e.from)
    ...
    const r = e.to ? (0, g.ensureAddressString)(e.to) : null,
      ...
      i = e.data ? (0, g.ensureBuffer)(e.data) : Buffer.alloc(0),
      a = e.nonce != null ? (0, g.ensureIntNumber)(e.nonce) : null,
      ...
      l =
        e.maxPriorityFeePerGas != null
          ? (0, g.ensureBigInt)(e.maxPriorityFeePerGas)
          : null,
  ...
  async _signEthereumMessage(e, t, r, s) {
    ...
    try {
      ...
      throw typeof i.message == "string" &&
        ...
        ? w.standardErrors.provider.userRejectedRequest(
  ...
  async _ethereumAddressFromSignedMessage(e, t, r) {
    const i = await this.initializeRelay().ethereumAddressFromSignedMessage(
      e,
      t,
      r
    );
  ...
  async _eth_requestAccounts() {
    ...
    try {
      if (
        ((e = await this.initializeRelay().requestEthereumAccounts()),
        (0, D.isErrorResponse)(e))
      )
  ...
  async _eth_sendRawTransaction(e) {
    const t = (0, g.ensureBuffer)(e[0]),
      s = await this.initializeRelay().submitEthereumTransaction(
        t,
        this.getChainId()
      );
    if ((0, D.isErrorResponse)(s)) throw new Error(s.errorMessage);
  ...
  async _eth_sendTransaction(e) {
    ...
    const t = this._prepareTransactionParams(e[0] || {});
    try {
      const s = await this.initializeRelay().signAndSubmitEthereumTransaction(
        t
      );
      if ((0, D.isErrorResponse)(s)) throw new Error(s.errorMessage);
      return { jsonrpc: "2.0", id: 0, result: s.result };
  ...
  async _eth_signTypedData_v1(e) {
    ...
    const s = Ke.default.hashForSignTypedDataLegacy({ data: t }),
  ...
  async _eth_signTypedData_v3(e) {
    ...
    this._ensureKnownAddress(t);
    const s = Ke.default.hashForSignTypedData_v3({ data: r }),
  ...
  async _eth_signTypedData_v4(e) {
    ...
    const t = (0, g.ensureAddressString)(e[0]),
      r = (0, g.ensureParsedJSONObject)(e[1]);
    this._ensureKnownAddress(t);
    const s = Ke.default.hashForSignTypedData_v4({ data: r }),
      i = JSON.stringify(r, null, 2);
    return this._signEthereumMessage(s, t, !1, i);
  }
  async _wallet_addEthereumChain(e) {
    ...
    if (!a.nativeCurrency)
      throw w.standardErrors.rpc.invalidParams(
        "nativeCurrency is a required field"
      );
    const o = parseInt(a.chainId, 16);
    return (await this.addEthereumChain(
      o,
      (r = a.rpcUrls) !== null && r !== void 0 ? r : [],
      (s = a.blockExplorerUrls) !== null && s !== void 0 ? s : [],
      a.chainName,
      (i = a.iconUrls) !== null && i !== void 0 ? i : [],
      a.nativeCurrency
    ))
  ...
  async _wallet_watchAsset(e) {
    ...
    return {
      jsonrpc: "2.0",
      id: 0,
      result: await this.watchAsset(t.type, s, i, o, a, r),
    };
  }
  initializeRelay() {
    if (!this._relay) {
      ...
      e.setAppInfo(this._appName, this._appLogoUrl),
        e.attachUI(),
        e.setAccountsCallback((t, r) => this._setAddresses(t, r)),
        e.setChainCallback((t, r) => {
...
var k = {};
Object.defineProperty(k, "__esModule", { value: !0 });
k.checkErrorForInvalidRequestArgs =
  k.getCoinbaseInjectedProvider =
  k.getCoinbaseInjectedSigner =
  k.fetchRPCRequest =
    void 0;
const Yr = K,
  pe = j;
async function Jr(n, e) {
  if (!e.rpcUrl)
    throw pe.standardErrors.rpc.internal("No RPC URL set for chain");
...
function es({ metadata: n, preference: e }) {
  var t;
  if (e.options !== "smartWalletOnly") {
    ...
    if (i) {
      ...
      return (
        (t = i.setAppInfo) === null || t === void 0 || t.call(i, a, o, c), i
      );
    }
  }
  const r = Xr();
  if (r != null && r.isCoinbaseBrowser) return r;
}
k.getCoinbaseInjectedProvider = es;
function ts(n) {
  if (!n || typeof n != "object" || Array.isArray(n))
    return pe.standardErrors.rpc.invalidParams({
      message: "Expected a single, non-array, object argument.",
      data: n,
    });
...
k.checkErrorForInvalidRequestArgs = ts;
...
const ns = _e,
  ...
  is = O,
  Jt = "SignerType",
  Qt = new is.ScopedLocalStorage("CBWSDK", "SignerConfigurator");
function as() {
...
P.fetchSignerType = cs;
function ds(n) {
  const { signerType: e, metadata: t, communicator: r, updateListener: s } = n;
  switch (e) {
    case "scw":
      return new ns.SCWSigner({
        metadata: t,
        updateListener: s,
        communicator: r,
      });
    case "walletlink":
      return new Yt.WalletLinkSigner({ metadata: t, updateListener: s });
    case "extension": {
      const i = (0, ss.getCoinbaseInjectedSigner)();
      if (!i) throw rs.standardErrors.rpc.internal("injected signer not found");
      return i;
    }
  }
}
...
async function ls(n, e) {
  ...
  const t = new Yt.WalletLinkSigner({ metadata: e });
  n.postMessage({
...
function hs(n) {
  const e = (window.innerWidth - pt) / 2 + window.screenX,
    ...
    r = window.open(
      n,
      "Smart Wallet",
      `width=${pt}, height=${ft}, left=${e}, top=${t}`
    );
  if ((r == null || r.focus(), !r))
    throw us.standardErrors.rpc.internal("Pop up window failed to open");
  return r;
}
J.openPopup = hs;
function gs(n) {
    ...
    (this.popup = null),
      ...
      (this.onMessage = async (t) =>
        new Promise((r, s) => {
          ...
          window.addEventListener("message", i),
            this.listeners.set(i, { reject: s });
        })),
      (this.disconnect = () => {
        (0, mt.closePopup)(this.popup),
          (this.popup = null),
          this.listeners.forEach(({ reject: t }, r) => {
            t(
              bt.standardErrors.provider.userRejectedRequest("Request rejected")
            ),
      ...
      (this.waitForPopupLoaded = async () =>
        this.popup && !this.popup.closed
          ...
          : ((this.popup = (0, mt.openPopup)(this.url)),
            this.onMessage(({ event: t }) => t === "PopupUnload")
              .then(this.disconnect)
              .catch(() => {}),
            this.onMessage(({ event: t }) => t === "PopupLoaded")
              .then((t) => {
                this.postMessage({
                  requestId: t.id,
                  data: { version: ps.LIB_VERSION },
                });
              })
              .then(() => {
...
class ks extends vs.default {
    ...
    super(),
      ...
      (this.handlers = {
        handshake: async (l) => {
          try {
            if (this.connected)
              return (
                this.emit("connect", {
                  chainId: (0, X.hexStringFromIntNumber)(
                    (0, he.IntNumber)(this.chain.id)
                  ),
                }),
                this.accounts
              );
        ...
        sign: async (l) => {
          if (!this.connected || !this.signer)
            throw z.standardErrors.provider.unauthorized(
        ...
        state: (l) => {
          ...
          switch (l.method) {
            case "eth_chainId":
              return (0, X.hexStringFromIntNumber)(
                (0, he.IntNumber)(this.chain.id)
              );
            case "net_version":
        ...
        deprecated: ({ method: l }) => {
          throw z.standardErrors.rpc.methodNotSupported(
            `Method ${l} is deprecated.`
          );
        },
        unsupported: ({ method: l }) => {
          throw z.standardErrors.rpc.methodNotSupported(
            `Method ${l} is not supported.`
          );
        },
      }),
      ...
      (this.updateListener = {
        onAccountsUpdate: ({ accounts: l, source: h }) => {
          (0, X.areAddressArraysEqual)(this.accounts, l) ||
            ((this.accounts = l),
            h !== "storage" && this.emit("accountsChanged", this.accounts));
        },
        onChainUpdate: ({ chain: l, source: h }) => {
          (l.id === this.chain.id && l.rpcUrl === this.chain.rpcUrl) ||
            ((this.chain = l),
            h !== "storage" &&
              this.emit(
                "chainChanged",
                (0, X.hexStringFromIntNumber)((0, he.IntNumber)(l.id))
              ));
        },
      }),
      (this.metadata = s),
      (this.preference = o),
      (this.communicator = new Es.Communicator(a)),
      (this.chain = {
        id:
          (r = (t = s.appChainIds) === null || t === void 0 ? void 0 : t[0]) !==
            null && r !== void 0
            ? r
            : 1,
      });
  ...
  async request(e) {
    ...
    try {
      ...
      if (r) throw r;
      const s =
        (t = (0, Ss.determineMethodCategory)(e.method)) !== null && t !== void 0
          ? t
          : "fetch";
      return this.handlers[s](e);
    } catch (r) {
      return Promise.reject((0, _s.serializeError)(r, e.method));
    }
  }
  ...
  async disconnect() {
    (this.accounts = []),
      (this.chain = { id: 1 }),
      Cs.ScopedLocalStorage.clearAll(),
      this.emit(
        "disconnect",
        z.standardErrors.provider.disconnected("User initiated disconnection")
      );
  }
...
class Ps {
  constructor(e) {
    (this.metadata = {
      ...
      appLogoUrl: e.appLogoUrl || (0, Rs.getFavicon)(),
  ...
  makeWeb3Provider(e = { options: "all" }) {
    ...
    return (t = (0, xs.getCoinbaseInjectedProvider)(r)) !== null && t !== void 0
      ? t
      : new Ms.CoinbaseWalletProvider(r);
  }
  getCoinbaseWalletLogo(e, t = 240) {
    return (0, Is.walletLogo)(e, t);
  }
  storeLatestVersion() {
    new Ls.ScopedLocalStorage("CBWSDK").setItem("VERSION", As.LIB_VERSION);
  }
}
te.CoinbaseWalletSDK = Ps;
(function (n) {
  Object.defineProperty(n, "__esModule", { value: !0 }),
